  ...
  if (status === 'authenticated') {
    return (
      <div className="flex items-center space-x-4">
        <span className="text-[#1A522A] font-medium hidden lg:inline-flex gap-3">
          <CircleUserRound /> {session.user?.name || session.user?.email?.split('@')[0]}!
        </span>
        <Link href="/dashboard">
            <Button variant="outline" className="border-[#1A522A] text-[#1A522A] hover:bg-[#1A522A] hover:text-white ease-in-out transition-colors duration-300">
                Dashboard
            </Button>
        </Link>
