...
export default function OnboardingPage() {
  ...
  useEffect(() => {
    ...
    console.log('Current isApproved (from session in Onboarding useEffect):', session?.user?.isApproved);
    ...
    if (status === 'unauthenticated') {
      ...
      if (session?.user?.isProfileComplete && session?.user?.isApproved) {
        console.log('Onboarding useEffect: Authenticated, Profile Complete, and Approved. Redirecting to /dashboard');
        router.push('/dashboard');
      } else if (session?.user?.isProfileComplete && !session?.user?.isApproved) {
        console.log('Onboarding useEffect: Authenticated, Profile Complete, but NOT Approved. Redirecting to /pending-approval');
      ...
      } else if (session?.user && session.user.isProfileComplete === undefined) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        alert('Profile completed successfully! Your account is now pending admin approval.'); // Updated alert message

        // The update() call will trigger a session refresh.
        // Once the session is refreshed and isProfileComplete is true,
        // the useEffect above will trigger the router.push('/pending-approval').
        // isApproved will still be false, as set in models/User and /api/register
        await update({
          user: {
            committee,
            portfolio,
            ...
            // Do NOT set isApproved here. It should remain false until admin approves.
          },
        });
        console.log('Onboarding form: Session update initiated. Expecting useEffect to re-run and redirect.');
        // DO NOT router.push('/pending-approval') here directly. Let the useEffect handle it.
      }
    } catch (err: any) {
  ...
  // Only show loading if session is truly loading or if profile is complete (and will redirect soon)
  ...
  // Only render the form if authenticated and profile is explicitly NOT complete
